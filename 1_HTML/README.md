...
```html
<img src = "이미지 경로" alt="이미지 설명 문구" width ="가로 길이(px/%)" height = "세로길이(px/%)">
```
...
```html
<video src= "비디오경로" controls autoplay loop width="" height="" poster="썸네일 이미지 경로">
```
...
- 텍스트를 클릭해 링크를 이동하는 방법, 이미지르르 클릭해서 넘어가는 방법 등 존재
...
1. **text** : 한 줄의 텍스트를 입력받을 수 있다.
      ...
      - maxlength : 최대 입력 가능한 글자수
      - minlength : 최소 입력 글자 수
      - value : 입력 값

2. **password** : 비밀번호를 입력받을 수 있다.(텍스트와 동일)
...
5. **file** : 파일을 첨부할 수 있다.
      ...
      - accpet : 파일 선택할 때 보여지는 파일들의 종류를 지정한다.
...
- **datetime** or **datetime-local** : 년/월/일/시/분/초에
대한 데이터를 입력받을 수 있다.
